...
var outputDirPath = join(__dirname, outputDir);
var recursive = true;
var encoding = "utf-8";
var dir = "xslt_desa";
var filesPath = join(__dirname, dir);
var deploymentFile = "deployment.yml";
var deploymentPath = join(__dirname, deploymentFile);
...
async function updateDeploymentWithVolumes(configMapNames) {
  try {
    const deploymentContent = await readFile(deploymentPath, encoding);
    const deployment = $parse(deploymentContent);
    ...
    podSpec.volumes = podSpec.volumes.filter((vol) => preservedVolumeNames.has(vol.name) || !vol.configMap);
    const baseMountPath = "/deployments/xslt_desa";
    container.volumeMounts = container.volumeMounts.filter((mount) => !mount.mountPath.startsWith(baseMountPath) || preservedVolumeNames.has(mount.name));
    ...
    const updatedDeploymentYAML = $stringify(deployment, {
      indent: 2,
      lineWidth: 0,
      minContentWidth: 0
    });
